...
D'autres fonctionnalités de \textit{GitHub} ont également été très utiles comme les \textit{Milestones} ou les \textit{issues}. Les premières permettent de poser une date de rendu de travail. Les issues quant à elles nous permettaient de préciser différents soucis dans la construction ou le développement du site, ou de détailler les fonctionnalités à ajouter au site. Par exemple, pour les résumés, il a été décidé dès le début de créer un formulaire dans la partie profil utilisateur, d'obliger les fichiers à être au format pdf, et bien d'autres précisions concernant le développement.
...
La récupération de mot de passe fonctionne sensiblement de la même façon que la validation par email. Un jeton est généré pour l'utilisateur, qui lui est envoyé par mail et permet à ce dernier de se connecter sans entrer son mot de passe. Ce dernier est détruit à l'utilisation. Mais l'utilisateur peut changer son mot de passe durant cette session.
...
La soumission des résumés par les utilisateurs fut donc ajoutée dans la partie profil. Il fallait faire en sorte que les utilisateurs entrent un résumé seulement. Après cela, la gestion des \textit{reviews} fut ajoutée dans la partie administrateur. L'affichage du fichier a été plus difficile à implémenter et fut faite plus tard. Néanmoins, l'administrateur put rapidement valider la proposition soumise, la refuser ou demander à l'utilisateur une nouvelle proposition. 

La suite fut la partie modération. Elle consiste en une page spécifique aux utilisateurs ayant le rôle de modérateur. L'administrateur leur assigne des résumés qu'il doit relire et valider ou non. Le même type de validation que dans la partie administrateur fut donc implémentée. Il fallait cependant que seule certain résumés soient disponibles à la relecture. L'administration a donc été dotée d'une page d'assignation.  Les modérateurs sont listés sur cette page, et le résumé sélectionné est assigné au modérateur choisi par l'administrateur (en fonction du domaine de compétence de ce dernier par exemple). Dans la partie modération, l'utilisateur peut donc valider les résumés assignés et uniquement ceux-ci.


\bigbreak
Enfin, le participant qui soumet un résumé, l'administrateur et le modérateur doivent recevoir des mails lorsqu'ils ont des notifications : nouveau résumé à corriger pour le modérateur ou validation pour le participant par exemple. La méthode d'envoi de mail fut ainsi réutilisée.
