...
\smallbreak
 Pour développer cette application, nous utilisons le langage \textit{Ruby}, aidé par le framework \textit{Sinatra}. La base de donnée est gérée par la bibliothèque SQL \textit{Sqlite3}, et l'ORM Activerecord. Le style utilise quant à lui \textit{Bootstrap}. 
...
Lors du développement de l'application, nous avons fait attention à utiliser les principes de qualité logiciel. Nous utilisons donc le gestionnaire de version \textit{GIT}, effectuons des tests fonctionnels et unitaire. Le site web \textit{travis } permet de s'assurer que notre application reste fonctionnelle. 
\end{abstract} 
...
Une fois le \textit{CRUD} de l'événement terminé, nous y avons ajouté la possibilité de créer un élément de formulaire. En effet, lorsque l'utilisateur s'inscrit à l'événement, on souhaite qu'il puisse remplir un formulaire pour ajouter des informations complémentaires.
