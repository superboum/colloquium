...
\smallbreak
Le site  permet à l'administrateur de créer différentes pages et article, mais aussi des événements auxquels les utilisateurs ont la possibilité de s'inscrire. Lors de son inscription, l'utilisateur les sélectionne donc. À partir de ces données, et d'informations fournies, un bon de commande est généré, qui permet à l'utilisateur d'achever cette inscription.
...
D'autres fonctionnalités de \textit{GitHub} ont également été très utiles comme les \textit{issues}. Les \textit{issues} nous permettaient de préciser différents soucis dans la construction ou le développement du site, ou de détailler les fonctionnalités à ajouter au site. Par exemple, pour les résumés, il a été décidé dès le début de créer un formulaire dans la partie profil utilisateur, d'obliger les fichiers à être au format pdf, et bien d'autres précisions concernant le développement.
...
Les événements comportent trois aspects : la description de l'événement, la gestion de l'inscription à l'événement, et la gestion des statistiques qui permettent de télécharger un fichier Excel, qui résumera toutes les inscriptions. 

\medbreak
%\subsection*{La création du formulaire d'inscription}
La première étape a consisté en la création du \textit{CRUD} de l'événement. Une fois celui-ci terminé, nous y avons ajouté la possibilité de créer un élément de formulaire. En effet, lorsque l'utilisateur s'inscrit à l'événement, on souhaite qu'il puisse remplir un formulaire pour ajouter des informations complémentaires.

Cette création est légèrement plus complexe que la création d'un article. L'élément doit être relié à un événement. Il s'agit donc d'une relation \textit{many to one}. \textit{Activerecord} simplifie normalement ce processus. Cependant, la configuration n'a pas été correctement effectuée dans un premier temps. Il fallait alors écrire une grande partie des requêtes \textit{SQL} à la main.

\medbreak
%\subsection*{La gestion des inscriptions}
Lorsque les éléments de formulaires furent correctement configurés, nous avons implémenté l'inscription à un événement.

Deux éléments sont à vérifier : l'utilisateur doit bien remplir les différents champs, et le site doit vérifier que le nombre limite de place n'a pas été dépassé.
\medbreak
...
Tous d'abord, les mots de passes sont hashés et salés dynamiquement. Le hash est à sens unique, et ne permet pas de retrouver le mot de passe initial. De plus, le fait de saler le mot de passe permet d'empêcher la personne malveillante de retrouver le mot de passe à partir d'une rainbow table. En effet, deux utilisateurs ayant le même mot de passe auront un hash différent grâce au salage dynamique.
